...
const ListView = React.createClass({
	...
	render () {
		...
		let canSeeThisPage=false
		try{
			for (let i = 0; i < Keystone.user.authModel.length; i++) {
				let authModelElement = Keystone.user.authModel[i]
				let path=Keystone.lists[authModelElement.code].path
